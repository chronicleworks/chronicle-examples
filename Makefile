...
$(1)-inmem-debug: $(MARKERS)/ensure-context-$(1)-inmem-debug domains/$(1)/domain.yaml $(1)-lint
	@echo "Building $(1) debug inmem as docker image chronicle-$(1)-inmem-$(ARCH_TYPE):$(ISOLATION_ID)"
	...
	@$(DOCKER_TAG) chronicle-domain:$(ISOLATION_ID) \
		chronicle-$(1)-inmem-$(ARCH_TYPE):$(ISOLATION_ID)

.PHONY: $(1)-stl
$(1)-stl-debug:$(MARKERS)/ensure-context-$(1)-stl-debug domains/$(1)/domain.yaml $(1)-lint
	@echo "Building $(1) debug chronicle stl as docker image chronicle-$(1)-stl-$(ARCH_TYPE):$(ISOLATION_ID)"
	@$(DOCKER_BUILD) -f docker/chronicle.dockerfile \
		...
		--load
	@$(DOCKER_TAG) chronicle-domain:$(ISOLATION_ID) \
		chronicle-$(1)-stl-$(ARCH_TYPE):$(ISOLATION_ID)

.PHONY: $(1)-inmem-release
$(1)-inmem-release: $(MARKERS)/ensure-context-$(1)-inmem-release domains/$(1)/domain.yaml $(1)-lint
	@echo "Building $(1) release inmem as docker image chronicle-$(1)-inmem-release-$(ARCH_TYPE):$(ISOLATION_ID)"
	@$(DOCKER_BUILD) -f docker/chronicle.dockerfile \
		...
		--load
	@$(DOCKER_TAG) chronicle-domain:$(ISOLATION_ID) \
		chronicle-$(1)-inmem-release-$(ARCH_TYPE):$(ISOLATION_ID)

.PHONY: $(1)-stl-release
$(1)-stl-release: $(MARKERS)/ensure-context-$(1)-stl-release domains/$(1)/domain.yaml $(1)-lint
	@echo "Building $(1) release chronicle stl as docker image chronicle-$(1)-stl-release-$(ARCH_TYPE):$(ISOLATION_ID)"
	@$(DOCKER_BUILD) -f docker/chronicle.dockerfile \
		...
		--load
	@$(DOCKER_TAG) chronicle-domain:$(ISOLATION_ID) \
		chronicle-$(1)-stl-release-$(ARCH_TYPE):$(ISOLATION_ID)

domains/$(1)/chronicle.graphql: $(1)-inmem
	@echo "Generating $(1) GraphQL schema in file: domains/$(1)/chronicle.graphql"
	@docker run --env RUST_LOG=debug chronicle-$(1)-inmem-$(ARCH_TYPE):$(ISOLATION_ID) \
...
.PHONY: run-$(1)
run-$(1): $(1)-inmem-debug
	-CHRONICLE_IMAGE=chronicle-$(1)-inmem-$(ARCH_TYPE) CHRONICLE_VERSION=$(ISOLATION_ID) $(DOCKER_COMPOSE) -f ./docker/chronicle-domain.yaml up --force-recreate

.PHONY: run-stl-$(1)
run-stl-$(1): $(1)-stl-debug
	export CHRONICLE_IMAGE=chronicle-$(1)-stl-$(ARCH_TYPE); \
...
clean-images-$(1): $(MARKERS)
	docker buildx rm ctx-$(ISOLATION_ID) || true
	docker rmi chronicle-$(1)-inmem-$(ARCH_TYPE):$(ISOLATION_ID) || true
	docker rmi chronicle-$(1)-inmem-release-$(ARCH_TYPE):$(ISOLATION_ID) || true
	docker rmi chronicle-$(1)-stl-$(ARCH_TYPE):$(ISOLATION_ID) || true
	docker rmi chronicle-$(1)-stl-release-$(ARCH_TYPE):$(ISOLATION_ID) || true
